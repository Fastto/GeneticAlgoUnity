...
namespace Evolution.Scripts.Legacy
{
    public class CellOld: MonoBehaviour
    {
        ...
        private void Divide()
        {
            ...
            {
                CellOld cellOld = Instantiate(CellPrefab, transform.position, Quaternion.identity).GetComponent<CellOld>();
                cellOld.SetGenome(_genome.Clone().Mutate());
                cellOld.SetEnergy(_energy/ChildrenQuantity);
            }
        }
